...
void LightsController::setLightsPinsByCurrentMode() {
  ...
  digitalWrite(digitalReversePin, isReverse ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);

  // Inner only brake light
  digitalWrite(digitalInnerBrakePin, isBreaking ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);  // Turned ON when breaking only.

  // Outer brake light, it is combined brake and day lights (dimmed).
  digitalWrite(digitalOuterBrakePin, lightsAreOn || isBreaking ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);  // Tirned on when lights are on or when breaking.
  digitalWrite(digitalOuterBrakeModePin, isBreaking ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);             // If breaking, shine on max, otherwise dimmed.
  ...
  if (isFullLightMode) {
    ...
    digitalWrite(digitalLight1Pin, AppConstants::BYTE_MAX);
    digitalWrite(digitalLight2Pin, AppConstants::BYTE_MAX);
    digitalWrite(digitalLight3Pin, AppConstants::BYTE_MAX);
    return;
  }
  ...
  digitalWrite(digitalLight1Pin, LIGHT_MATRIX[currentLightMode][0] == 1 ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);
  digitalWrite(digitalLight2Pin, LIGHT_MATRIX[currentLightMode][1] == 1 ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);
  digitalWrite(digitalLight3Pin, LIGHT_MATRIX[currentLightMode][2] == 1 ? AppConstants::BYTE_MAX : AppConstants::BYTE_MIN);
}
